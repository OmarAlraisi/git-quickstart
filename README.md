# Git Quickstart

## Description

This workshop is a hands-on introduction to Git and GitHub. By the end, you'll be comfortable with the everyday Git workflow and the core GitHub collaboration features.

### Part 1 - Configure GitHub Repository

- Add collaborators
- Protect default branch

### Part 2 — Git Basics

We'll walk through the essential commands you'll use every day:

- `git clone` — copy a repository to your machine
- `git pull` — fetch and merge the latest changes
- `git status` — see what's changed
- `git add` — stage your changes
- `git commit` — save a snapshot of your work
- `git push` — share your changes with the remote

### Part 3 — Collaborating on GitHub

Next, you'll put those commands to work by making small contributions to this `README.md` file. Along the way you'll learn how to:

- Create **issues** to track work
- Create **branches** to work on changes safely
- Open **pull requests (PRs)** to propose your changes
- **Resolve merge conflicts** when they come up

The goal of this part isn't to write real documentation — it's just to practice the workflow. So your contribution can be as simple as **adding your name to the list below**.

### Participants

<!-- Add your name to the list! -->
- Hadil Al Maawali

-Tuqa AlHosni

- Omar Al Raisi

That's it — let's get started!
